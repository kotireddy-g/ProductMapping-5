...
const ForecastInternalDetailsScreen = ({ forecastData, onBack, selectedForecastArea }) => {
    ...
    // Get data from API or fallback to empty arrays
    const departments = apiData?.internal_departments || [];
    const medicines = apiData?.medicines || [];
    const whatIfScenarios = apiData?.what_if_scenarios || {};
    const recommendations = apiData?.agent_recommendations || [];

    // Get What-If results based on selected scenario
    const getWhatIfResults = () => {
        // Show results based on scenario selection, medicine selection is optional
        if (!whatIfScenarios[whatIfScenario]) return null;
        return whatIfScenarios[whatIfScenario].metrics;
    };


    // Internal departments based on selected forecast area
    // This function is no longer needed as departments are fetched from API
    ...
    // const departments = getInternalDepartments(); // departments are now from apiData
            ...
            {loading && (
                <div className="max-w-[1600px] mx-auto px-6 py-12">
                    <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-12 text-center">
                        ...
                        <p className="text-slate-600 font-semibold">Loading forecast data...</p>
                    </div>
                </div>
            )}
                                ...
                                <tbody className="divide-y divide-slate-200">
                                    {departments.map((dept, index) => (
                                        <tr key={dept.id} className={`hover: bg - blue - 50 transition - colors ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50'} `}>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <span className="font-bold text-purple-700 text-lg">{dept.stock_available}</span>
                                            </td>
                                            <td className="px-6 py-4 text-center">
                                ...
                                <div className="space-y-3">
                                    <label className="flex items-center gap-3 p-4 border-2 border-slate-300 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
                                        <input
                                            ...
                                            name="scenario"
                                            value="do_nothing"
                                            checked={whatIfScenario === 'do_nothing'}
                                    ...
                                    <label className="flex items-center gap-3 p-4 border-2 border-slate-300 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
                                        <input
                                            type="radio"
                                            name="scenario"
                                            value="order_additional"
                                            checked={whatIfScenario === 'order_additional'}
                                            onChange={(e) => setWhatIfScenario(e.target.value)}
                                            className="w-5 h-5 text-blue-600"
                                        />
                                    ...
                                    <label className="flex items-center gap-3 p-4 border-2 border-slate-300 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
                                        <input
                                            ...
                                            name="scenario"
                                            value="wait_7_days"
                                            checked={whatIfScenario === 'wait_7_days'}
                                            onChange={(e) => setWhatIfScenario(e.target.value)}
                                            className="w-5 h-5 text-blue-600"
                                        />
                        ...
                        <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
                            ...
                            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                                {recommendations.map((rec) => {
                                    // Map color_scheme to CSS classes
                                    const colorClasses = {
                                    ...
                                    const colorClass = colorClasses[rec.color_scheme] || 'bg-gray-50 border-gray-500';

                                    return (
                                        <div key={rec.id} className={`p-5 rounded-xl border-2 ${colorClass} hover:shadow-lg transition-all`}>
